...
export const WorkerDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const statsRes = await apiClient.get('/workers/dashboard');
  ...
  const handleToggleOnline = async () => {
    ...
    try {
      await apiClient.put('/workers/availability', { isOnline: nextState });
  ...
  const handleAcceptJob = async (bookingId: string) => {
    try {
      await apiClient.post(`/workers/jobs/${bookingId}/accept`);
      loadDashboardData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDeclineJob = async (bookingId: string) => {
    try {
      await apiClient.post(`/workers/jobs/${bookingId}/decline`);
      loadDashboardData();
    } catch (err) {
      console.error(err);
    }
  ...
  const handleStepStatusUpdate = async (bookingId: string, nextStatus: string) => {
    try {
      await apiClient.put(`/workers/jobs/${bookingId}/status`, { status: nextStatus });
      loadDashboardData();
    } catch (err) {
      console.error(err);
    }
